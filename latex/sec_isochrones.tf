...
Isochrone ridgeline residuals are shown in figure \ref{fig:m36_resid}. Due to the young age of M36 (30 Myr), not all isochrone systems will have a similar age. Padova and PARSEC cover the young age range, and 30 Myr isochrones were used in the comparison. Y$^2$'s youngest age is 100 Myr; Y$^2$ isochrones contain a turn-off which does not exist in the observed cluster, leading to inflated residuals above $V \sim 12$. After interpolating Dartmouth isochrones, the youngest available age is 1 Gyr, too much older than the observed cluster to produce any useful insight. Dartmouth isochrones are therefore not shown in figure \ref{fig:m36_resid}.

M36 does not have a published metallicity \citep{2002AA...389..871D}, so it was compared against solar-metallicity isochrones. As metallicity has one of the largest effects on isochrone ridgeline shape, much of the measured deviation may come from a metallicity mismatch. Until detailed chemical abundance studies are undertaken, better isochrone fits will not be available.

%-- Isochrones do not deviate much between systems or atmospheres.
%-- Lejuene color transformation in B-V is insanely off.
%-- Optical - IR colors are extremely discrepant, predicting much bluer stars.
...
%-- U magnitude is highly discrepant.
%-- All isochrones agree closely, but hover just blue of observed.
%-- BT-Settl atmospheres seem to make the already-bad optical-IR colors worse.
%-- Strange bump near V ~ 12 in all systems and atmospheres
...
%-- Except for NIR colors, which have a very small range to begin with, Padova and PARSEC isochrones deviate greatly to the blue.
%-- For most CMDs, Dartmouth matches observation to within 0.2, especially with BT-Settl atmospheres
%-- BT-Settl atmospheres seem to slightly improve the fits of Y^2 and Dartmouth sets.
...
%-- BT-Settl atmospheres accurately predict u magnitudes, which are greatly off in for other atmospheres.
%-- BT-Settl atmospheres seem to improve all system fits.
%-- As before, systems consistently predict bluer colors.
...
%-- u magnitudes are not accurately predicted by any isochrone / atmosphere combination.
%-- Dartmouth + BT-Settl isochrones fit excellently, to within 0.1 for nearly all CMDs
%-- Use of ATLAS9 atmospheres make the fit worse over original atmosphere models
