...
\item \textbf{Synthetic best-fit mass ratio:} After each run of the \binocs code on the cluster dataset, a fit against the full synthetic library is run, similar to that in \S\ref{sec:filters_test}. Using best-fit mass results of single stars in the synthetic library, an average best-fit mass ratio can be determined, as a function of primary mass.
...
\caption[Comparison of \binocs and RV-derived mass ratios for NGC 188]{\emph{Left:} NGC 188 CMD in $B-V$. Solid line is isochrone used to generate models for \binocs fitting. Dashed line is equal-mass binary sequence. Black circles are double-lined binaries. \emph{Right:} Comparison of RV mass ratios (black) from \citet{2009AJ....137.3743G} to \binocs (grey) for NGC 188 double-lined binaries. \label{fig:rv_n188}}
\end{figure}
