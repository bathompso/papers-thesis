...
Original star counts, as well as the number of stars available after the `222' cut are shown for all clusters in the analysis set in table \ref{tab:222starcounts}. In many cases, the individual photometry datasets cover different areas around the cluster, only overlapping in smaller regions. These extra sources, which have photometry from only one source, are removed after the filter cut. This effect is enhanced in M35, M36 and M37, which lie in the plane of the galaxy. 
