\section{Data Overview} \label{sec:data}
...
\subsubsection{Magnitude Measurements} \label{sec:mag_measuring}
...
\begin{figure}
\begin{center}
\includegraphics[width=0.37\textwidth]{../plots/jpg/AperturePhotometry.jpg}\vspace{10px}
\parbox{0.7\textwidth}{\caption{Aperture photometry set-up for a fictional star. Blue is image circle, green indicates sky annulus. A dense field may result in stars falling within the sky annulus. \label{fig:aperturephot}}}
\end{center}
\end{figure}
...
The benefit of this technique is that it is simple to do, and fast to compute. The only input parameters are the three circle radii, and a computer can determine magnitudes for thousands of stars very quickly. This technique breaks down, however, when stars are too close together for any sky annulus to accurately determine sky photon counts, or if the field is so dense that stars begin to overlap. For sparse fields, aperture photometry is the most efficient way to determine magnitudes. \\
...
\parbox{0.7\textwidth}{\caption[Distribution of Project Cluster Ages and Metallicities]{Distribution of 18 project clusters in age and [Fe/H] \citep{2002AA...389..871D}. X's represent clusters which do not have any published metallicity information. Points in grey correspond to clusters with ages $< 500$ Myr, usable for primordial binary studies. \label{fig:clusterParameters}}}
\end{figure}
...
\end{tabular}
\caption{Cluster parameters for all clusters in dataset \citep{2002AA...389..871D}. \label{tab:clusterParameters}}
\end{table}
...
\subsubsection{Optical Data Sources [0.3 -- 1.0 $\mu$m]}
...
\caption{Reduced MOSAIC 50s $I$-band image for M36.\label{fig:mosaic_image}}
\end{figure}

Three of the clusters have already been analyzed: M35 by Sarajedini \& Kinemuchi (\emph{private communcation}), and M36 and M38 at TCU. For the TCU analysis, the four images in each filter were combined to form a higher signal-to-noise master image, and to provide a complete covering of the cluster. Note the wide gap between chips in the individual MOSAIC images, shown in figure \ref{fig:mosaic_image}. Each of the four images per filter were \emph{dithered}, slightly offset, such that the combined image had no gaps in coverage. 
...
\caption{MOSAIC photometric quality plots for M36 in $UBVI$. \emph{Left:} Short set of exposures. \emph{Right:} Long set of exposures. \label{fig:mosaic_quality}}
\end{figure}
...
\end{tabular}
\caption{Transformation coefficients for MOSAIC photometry.\label{tab:mosaic_coeffs}}
\end{table}
...
\begin{figure} \centering
\includegraphics[width=0.49\textwidth]{../plots/eps/mosaic_resid.eps}
\parbox{0.7\textwidth}{\caption{Residuals from transformation to standard system for M36 MOSAIC photometry. \label{fig:mosaic_resid}}}
\end{figure}
...
\parbox{0.7\textwidth}{\caption[NEWFIRM 4Q Description]{Diagram of NEWFIRM's ``4Q'' mode. Cluster is centered in each of the four NEWFIRM chips for each image. \label{fig:newfirm_4q}}}
\end{figure}
...
\multirow{6}{*}{26 Nov 2013} & \multirow{3}{*}{NGC 188} 
	...
	& \multirow{3}{*}{NGC 2158} 
		...
		& & $K_S$ & 15$\times$60s & \\ \hline
\end{tabular}
\caption{NEWFIRM observing logs for all runs. Original 2008 data taken in ``4Q'' mode, while all other clusters were small enough to fit into a single NEWFIRM field of view. \label{tab:newfirm_obs}}
\end{table}
...
\caption{\emph{Left:} Plot of NEWFIRM photometric quality for the cluster M37. \emph{Right:} Plot of residuals in the transformation between ALLSTAR-derived magnitudes and 2MASS standards for M37. \label{fig:newfirm_phot}}
\end{figure}

NEWFIRM instrumental magnitudes were tied to the standard system using 2MASS photometry. The NEWFIRM sources were matched to `AAA'-quality 2MASS sources, meaning the magnitude measurement was deemed the best level possible in all 3 bands. The number of overlapping stars was significant for each of the reduced clusters, varying between 700 and 2000 stars, depending on cluster density and image spatial coverage. Using these overlapping stars, transformation equations were determined:
...
\end{tabular}
\caption{Transformation coefficients for NEWFIRM photometry. \label{tab:newfirm_coeffs}}
\end{table}
...
\caption{Residuals between IRAC [3.6][4.5] magnitudes and WISE [3.4][4.6]. $\Delta[3]$ indicates residuals between the 3 $\mu$m bands of each instrument, while $\Delta[4]$ indicates the difference in 4 $\mu$m magnitudes. Black dots indicate stars where the transformation is valid ([3.4]$<14$ and [4.6]$<13.5$), while grey dots indicate stars outside the transformation region.\label{fig:iracwise}}
\end{figure}
...
NGC 2420					& \citet{2009ApJ...700..523A}	& 									&								\\ \hline
...
\end{tabular}
\caption{Available data for all clusters in this project. Names in italics represent observed data that has not yet been reduced. \label{table:clusters}}
\end{table}
