...
m_\text{Single} - m_\text{Binary} = -2.5 \log_{10} \left( \frac{F_\text{Single}}{F_\text{Binary}} \right) = -2.5 \log_{10} \left( \frac{F_\text{Single}}{2 F_\text{Single}} \right) = 0.753
\end{equation}
...
\noindent \textbf{Multiple Visits:} Stellar spectra must be obtained at least 3 times in order to determine whether a star displays shifting velocities. Fully characterizing the orbits of both stars requires many repeat observations, depending on the orbital period of the system. This multiplies the necessary telescope time for a quality RV study in order to detect and classify binaries. \\
...
\noindent \textbf{Inclination \& Orbital Period:} Mass determination using double-lined binaries is complicated by the inclination of the system. While RV studies can determine the orbital mechanics of the system, they cannot accurately determine masses for all binaries. Without accurate masses for a large percentage of binary systems within the cluster, our science goals cannot be achieved. In addition, RV studies are limited by the resolution of their spectrometer; systems with doppler shifts less than the resolution of the spectrometer will not be registered as variable. Only binary systems with short enough orbital periods will produce large enough doppler shifts to be detected. \\
